...
public class SoTietKiemRepository : ISoTietKiemRepo
{
        ...
        bool? dangMo)
    {
        ...
        if (ngayGuiPhieuRutTienTu.HasValue)
            query = query.Where(stk => stk.DsPhieuRutTien.Any(prt => prt.NgayRut >= ngayGuiPhieuRutTienTu.Value));

        if (ngayGuiPhieuRutTienDen.HasValue)
            query = query.Where(stk => stk.DsPhieuRutTien.Any(prt => prt.NgayRut <= ngayGuiPhieuRutTienDen.Value));
        ...
        query = query.Where(stk => stk.DangMo == (dangMo ?? stk.DangMo));
